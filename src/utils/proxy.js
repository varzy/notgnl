...
const initProxy = () => {
  ...
  // 如果直接提供了代理路径
  if (process.env.NOTGNL_PROXY_ADDRESS) {
    proxyAddress = process.env.NOTGNL_PROXY_ADDRESS;
  }

  // 如果在 WSL 环境
  if (process.env.NOTGNL_USE_WSL) {
    const { stdout } = Shell.exec(`cat /etc/resolv.conf | grep nameserver | awk '{ print $2 }'`);
    const hostIP = stdout.trim();
    const port = process.env.NOTGNL_USE_WSL_PORT;
